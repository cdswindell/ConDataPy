...
class TestTableContext(TestBase):
    ...
    def test_indexed_table_ident(self) -> None:
        ...
        assert t.ident == 1
        t_ident = t.ident
        assert tc.get_table(ident=t_ident) == t
        assert t.ident in tc._table_ident_map
        assert tc._table_ident_map[t.ident] == t
        ...
        assert tc.get_table(ident=t_ident) is None  # type: ignore
        assert t_ident not in tc._table_ident_map
    ...
    def test_indexed_table_uuid(self) -> None:
        ...
        assert t is None  # type: ignore
        assert tc.get_table(uuid=t_uuid) is None  # type: ignore
